...
class AisConsentSpiImplTest {
    ...
    void authorisePsu() throws IOException {
        ...
        doReturn(scaConsentResponseTO).when(consentDataService)
                .response(CONSENT_DATA_BYTES, SCAConsentResponseTO.class, false);

        doReturn(scaConsentResponseTO).when(consentDataService)
                .response(CONSENT_DATA_BYTES);
    ...
    void authorisePsu_consentInternalError() throws IOException {
        ...
        when(authorisationService.authorisePsuForConsent(spiPsuData, password, CONSENT_ID, OpTypeTO.CONSENT, spiAspspConsentDataProvider))
                .thenReturn(SpiResponse.<SpiAuthorisationStatus>builder().payload(SpiAuthorisationStatus.SUCCESS).build());
        doReturn(scaConsentResponseTO).when(consentDataService).response(CONSENT_DATA_BYTES, SCAConsentResponseTO.class, false);
        doReturn(scaConsentResponseTO).when(consentDataService).response(CONSENT_DATA_BYTES);
    ...
    void authorisePsu_failureStatus() {
        ...
        // Then
        assertFalse(actualResponse.hasError());
        assertEquals(SPI_AUTHORISATION_STATUS_FAILURE, actualResponse.getPayload());
